...
use integrations::{esphome::apollo_air::create_apollo_air_1, Integration, IntegrationService};
use logging;
...
async fn main() -> Result<()> {
    ...
    // Clone event_bus before creating system
    let event_bus_clone = event_bus.clone();
    let timeseries_service_clone = timeseries_service.clone();
    // Create the system
