 ...
 * Aligned with ha-pro.md schema (event/eventType/eventPayload tables)
...
// Maps to the event/eventType/eventPayload tables in the database
...
export function getLogLevel(eventType: HubEventType): 'error' | 'warn' | 'info' | 'debug' {
	...
	if (eventType.includes('started') || eventType.includes('stopped') || eventType.includes('connected')) {
		return 'info';
	}
	return 'debug';
}
