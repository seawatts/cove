...
const log = debug('cove:api:hub:trpc');

/**
 * Initialize tRPC instance with hub context
 */
const t = initTRPC.context<HubContext>().create({
...
const timingMiddleware = t.middleware(async ({ next, path }) => {
  ...
  const end = Date.now();
  log(`[HUB TRPC] ${path} took ${end - start}ms to execute`);
  return result;
});
...
/**
 * Router creator for hub API
 */
export const createHubRouter = t.router;
