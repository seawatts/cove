...
describe('Device Discovery and Pairing', () => {
  ...
  it('should discover ESPHome device', async () => {
    ...
    if (daemon.getRegistry()) {
      const home = await daemon.getRegistry().getOrCreateHome('Test Home');
      const device = await daemon
        .getRegistry()
        .upsertDevice(deviceDesc, home.id);
  ...
  it('should pair device with credentials', async () => {
    ...
    if (daemon.getRegistry() && daemon.getDriverRegistry()) {
      const home = await daemon.getRegistry().getOrCreateHome('Test Home');
      const device = await daemon
        .getRegistry()
        .upsertDevice(deviceDesc, home.id);
      ...
      // Get the mock driver
      const driver = daemon.getDriverRegistry().get('esphome');
      ...
      const pairedDevice = await daemon.getRegistry().getDevice(device.id);
