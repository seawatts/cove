    ...
    .query(async ({ ctx, input }) => {
      ...
      const configs = await db.query.alertConfigs.findMany({
        where: (configs, { and, eq }) => {
          const conditions = [];
          if (input.entityId) conditions.push(eq(configs.entityId, input.entityId));
          if (input.field) conditions.push(eq(configs.field, input.field));
          if (input.homeId) conditions.push(eq(configs.homeId, input.homeId));
