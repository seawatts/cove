...
import { debug, error as logError, info } from '@cove/logger';
import { and, desc, eq, isNull } from 'drizzle-orm';
import type { DatabaseClient } from '../db';
import { alertConfigs, alertHistory } from '../db';
...
export class AlertService {
  ...
  private checkRange(
    ...
    const threshold = value < config.rangeMin ? config.rangeMin : config.rangeMax;
  ...
  private async checkRateOfChange(
    ...
    const newest = tracking.values[tracking.values.length - 1];
